/**
 * Error codes and messages for Parsian Payment Gateway
 */
 ...
 */
export function codeToMessage(code, message = '') {
  const codeStr = code.toString();
  let result = message;
  
  if (ERROR_CODES[codeStr]) {
    result = ERROR_CODES[codeStr];
    if (codeStr === '-1') {
  ...
  if (!result || result.trim() === '') {
    result = `خطای تعریف نشده! کد خطا: ${code}`;
  }
...
/**
 * Custom Parsian Payment Exception
 */
export class ParsianErrorException extends Error {
  constructor(code, message = '') {
    const errorMessage = codeToMessage(code, message);
    super(errorMessage);
    this.name = 'ParsianErrorException';
    this.code = code;
